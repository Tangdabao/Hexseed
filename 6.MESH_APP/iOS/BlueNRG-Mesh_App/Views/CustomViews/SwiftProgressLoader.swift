...
var coverViewTag = 9999

public class SwiftProgressLoader: UIView {
    ...
    class var sharedInstance: SwiftProgressLoader {
        struct Singleton {
            static let instance = SwiftProgressLoader(frame:CGRect(x: 0, y: 0, width: Config().size, height: Config().size))
    ...
    public class func show(animated: Bool) {
        self.show( title: nil,currentView:UIView(), animated: animated)
    }
    
    public class func show(title: String?,currentView:UIView,  animated : Bool) {
        //let currentWindow : UIWindow = UIApplication.shared.keyWindow!
        
        let loader = SwiftProgressLoader.sharedInstance
        loader.canUpdated = true
        loader.animated = animated
        loader.title = title
        loader.update()
        
        let height : CGFloat = currentView.frame.size.height //UIScreen.main.bounds.size.height
        let width : CGFloat = currentView.frame.size.width
        let center : CGPoint = CGPoint(x: width / 2.0, y: height / 2.0)
        loader.center = center
        
        loader.coverView = UIView(frame: currentView.bounds)
        loader.coverView?.backgroundColor = UIColor.gray
        loader.coverView?.alpha = 0.3
        
        loader.coverView?.tag = coverViewTag
        coverViewTag = coverViewTag + 1
        if (loader.superview != currentView) {
            currentView.addSubview(loader.coverView!)
    ...
    private func start() {
        ...
        SwiftProgressLoader.isShown = true
    }
    ...
    private func stop() {
        ...
        SwiftProgressLoader.isShown = false
    }
    ...
    public struct Config {
         ...
         */
        public var spinnerColor = UIColor(red: 89.0/255.0, green: 165.0/255.0, blue: 214.0/255.0, alpha: 1.0)
