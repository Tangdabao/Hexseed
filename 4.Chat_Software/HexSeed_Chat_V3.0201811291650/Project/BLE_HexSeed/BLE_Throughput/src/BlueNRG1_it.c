...
void SysTick_Handler(void)
{
    ...
    if(MsCouter==5)//如果有数据输入并且超过5ms，代表20字节传输完毕
    {
        APP_FLAG_SET(TX_BUFFER_FULL);
        MsCouter = 0;//清除计数器
        Uart_Timer_En=0;//关掉计数器
    }
    //预防计数超时
...
void MFT2A_Handler(void)
{
    ...
    {
        ADCCouter++;
        if (ADCCouter==250)//200ms上报一次ADC（4ms）
        {
            ADCCouter=0;
...
void MFT1B_Handler(void)
{
    if ( MFT_StatusIT(MFT1, MFT_IT_TND) != RESET )
    {
        ...
        if(CmdFromBt_st.BTCmdCount > 10)//如果50ms内无数据在发送，则完成一帧数据
        {
            //System_flag.Com_Flag=1;
            CmdFromBt_st.BTFinalFlag = 1;//接收完成标志
            CmdFromBt_st.BTCmdCount = 0;//清除计数缓存

            SdkEvalTimer_Enable(DISABLE);//关闭定时器
