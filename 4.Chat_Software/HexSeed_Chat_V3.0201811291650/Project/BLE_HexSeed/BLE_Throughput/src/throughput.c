...
void TempDatatophone(uint8_t *data, uint8_t data_length)
{
    ...
    if(APP_FLAG(CONNECTED) && APP_FLAG(NOTIFICATIONS_BAT_ENABLED))
    {
        do
        {
            BleState = aci_gatt_update_char_value(BattServHandle, BattTXCharHandle, 0, data_length, data);//把数据上发到手机端
        }
...
void BattDatatophone(uint8_t *data, uint8_t data_length)
{
    ...
    if(APP_FLAG(CONNECTED) && APP_FLAG(NOTIFICATIONS_BAT_ENABLED))
    {
        do
        {
            BleState = aci_gatt_update_char_value(BattServHandle, LowStatusTXCharHandle, 0, data_length, data);//把数据上发到手机端
        }
        while(BleState == BLE_STATUS_INSUFFICIENT_RESOURCES);
    }
}
...
void APP_Tick(void)
{
    ...
    if(APP_FLAG(RX_BUFFER_FULL))//如果手机下发数据满或者时间超时，数据发送Uart
    {
...

{
    ...
    APP_FLAG_CLEAR(RX_BUFFER_FULL);//连接之后，先清除接收数据标志
    CmdFromBt_st.BTCmdLenth = 0;//发送完毕之后，清除缓存
    Osal_MemSet(CmdFromBt_st.BTCmdBuff, 0, 128);//清除Buff缓存
    SdkEvalLedOn(LED2_R);
